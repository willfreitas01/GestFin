...
import { ListTransactionsCategory } from "@workspace/api-client-react/src/generated/api.schemas";
...
export default function Historico() {
  ...
  // Use undefined for category when "todos" is selected
  const categoryParam = filter === "todos" ? undefined : (filter as ListTransactionsCategory);
  
  const { data: transactions, isLoading } = useListTransactions({ category: categoryParam });
  ...
  const filters = [
    ...
    { value: "venda", label: "Receita Operacional" },
    { value: "material", label: "Insumos" },
    { value: "funcionarios", label: "Folha de Pagamento" },
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Todas as suas movimentações financeiras</p>
      </div>
        ...
        <CardContent className="p-0 sm:p-6">
          {isLoading ? (
            ...
            <div className="divide-y">
              {transactions.map((tx) => (
                <div key={tx.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-muted/30 transition-colors gap-3">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <Badge variant="outline" className={`text-xs font-normal px-2 py-0.5 ${CATEGORY_COLORS[tx.category] || ''}`}>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Não há transações para os filtros selecionados.</p>
            </div>
          )}
        </CardContent>
      </Card>
